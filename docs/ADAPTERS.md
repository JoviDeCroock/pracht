...
Adapters bridge previte's platform-agnostic core to specific deployment targets.
...
Platform Request (e.g. Node IncomingMessage, CF Worker fetch)
  ...
  → Adapter checks: is this a static asset?
    ...
    → No: is this a prerendered SSG/ISG page?
      ...
      → No: delegate to handlePreviteRequest()
...
The factory returns a `PreviteAdapter` object that the Vite plugin uses to
...
(`virtual:previte/server`) that bootstraps the server.
...
| --------------- | -------------------- | ----------------------------------- |
| `app`           | `PreviteApp`           | The resolved app from `defineApp()` |
...
- **ISG revalidation**: checks `previte-isg-manifest.json` for time revalidation
...
```javascript
// virtual:previte/node-server (generated)
...
Running `previte build` for a Node target emits `dist/server/server.js`, which is
...
| Option          | Type                                        | Description                               |
| --------------- | ------------------------------------------- | ----------------------------------------- |
| `app`           | `PreviteApp`                                  | The resolved app                          |
...
executionContext }` to previte so loaders, actions, and middleware can access
  bindings without extra wiring.
- **Build output**: `previte({ adapter: cloudflareAdapter() })` makes `previte build`
...
previte's virtual server module via `@cloudflare/vite-plugin` — no extra
files needed.
...
```javascript
// virtual:previte/server (generated in cloudflare mode)
import { handlePreviteRequest, resolveApp, resolveApiRoutes } from "previte";
...
- **Build Output API v3**: `previte({ adapter: vercelAdapter() })` makes `previte build`
...
```javascript
// virtual:previte/server (generated in vercel mode)
import { handlePreviteRequest, resolveApp, resolveApiRoutes } from "previte";
import { app } from "./src/routes.ts";
...
export function myAdapter(options?: MyOptions): PreviteAdapter {
  return {
    id: "my-platform",
    ...
    createServerEntryModule() {
      ...
      // generated virtual:previte/server module.
      return `
