...
- **CLI production builds** — `packages/cli/src/build.ts` owns Vite client/server
  builds, output normalization, prerender execution, generated artifacts,
  adapter finalization, and budget analysis. The Citty command only maps build
  flags, while preview invokes the same service directly.
- **CLI production preview** — `packages/cli/src/preview.ts` validates projects,
  resolves existing or newly built adapter output, and creates the production
  process plan. Shared built-in adapter inference lives in `adapter-target.ts`;
  the Citty command owns presentation, child-process lifecycle, and exit codes.
...
- **CLI** — `pracht dev` starts a Vite dev server with SSR, `pracht build` runs
  ...
  wiring, API discovery, and pages-router checks live in focused
  `verification-*-checks.ts` modules. Shared Markdown transform detection lives
  in `verification-markdown-checks.ts` rather than depending on either routing
  mode. Middleware export source analysis lives
...
- **Test utilities** — `@pracht/test` keeps `args.ts` as the stable factory
  facade. Cross-realm `Request` construction lives in `request.ts`, public args
  contracts in `args-types.ts`, and route/API metadata assembly in
  `args-factories.ts`; form helpers import those narrow layers directly.
