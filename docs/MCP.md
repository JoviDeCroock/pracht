...
| `inspect_capabilities` | `cwd?`                                                                                                          | Registered capabilities: name, effect class, exposure transports, HTTP path, middleware, source file. Same as `pracht inspect capabilities --json`. |
...
| `verify`              | `cwd?`, `changed?` (boolean, maps to `--changed`)                                                                   | Framework verification checks with scope info. Same as `pracht verify --json`.       |
| `generate_route`      | `cwd?`, `path`, `render?` (`spa`/`ssr`/`ssg`/`isg`), `shell?`, `middleware?` (string[]), `loader?`, `errorBoundary?`, `staticPaths?`, `title?`, `revalidate?` (seconds) | Files created and updated (`{ kind, created, updated }`).                            |
