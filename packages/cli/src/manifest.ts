...
export function extractRegistryEntries(source: string, key: string): RegistryEntry[] {
...
function insertBlockEntry(source: string, block: BlockInfo, entry: string): string {
  const inner = source.slice(block.openIndex + 1, block.closeIndex);
