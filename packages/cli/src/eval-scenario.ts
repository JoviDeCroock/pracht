/**
 * Declarative scenario model for `pracht eval`: discovery, validation,
 * cross-step references, and expectation matching. This layer never performs
 * network requests, so format changes remain independent from execution.
 */

import { readdirSync, readFileSync, statSync } from "node:fs";
import { join, resolve } from "node:path";
import type { AgentSigningJwk } from "@pracht/core/agent-auth";
