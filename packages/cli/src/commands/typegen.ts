...
function buildDeclarationSource(
  ...
  const typeImports = [
    ...
    ...(hasUntypedSearch ? ["SearchParamsInput", "SearchParamsRecord"] : []),
  ];
  const lines = [
  ...
  for (const [index, route] of routes.entries()) {
    // The `search` schema is a route-module export (never the separate
    // loader file), so both sides of it come from the route module.
    ...
    lines.push(
      routeModule
        ...
        : "        searchOutput: SearchParamsRecord;",
    );
