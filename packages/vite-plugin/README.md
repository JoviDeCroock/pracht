...
For contributors, `src/plugin-codegen.ts` is the stable virtual-module
generator facade. Client hydration and islands bootstraps live in
`plugin-client-codegen.ts`, server bundle assembly in
`plugin-server-codegen.ts`, and the adapter-neutral inspection graph in
`plugin-dev-codegen.ts`; the shared generated route-loader hint helper lives in
`plugin-codegen-route-hints.ts`. Lazy route/API/capability registry globs are generated by
`plugin-registry-codegen.ts`, while `plugin-llms-txt-config.ts` owns package
