 ...
 * the effect-derived annotation policy shared with the remote MCP
 * projection — lives in `@pracht/capabilities/webmcp` (published, so
 * non-pracht sites register tools with identical semantics); this module only
 * contributes the statically extracted tool metadata and the app-specific
 * dispatch. `@pracht/capabilities` is resolvable here by construction: an app
 * only gets this virtual module once it registers capability modules, which
 * import `defineCapability` from that package.
 */
export function createPrachtWebmcpModuleSource(
  ...
  const tools = capabilities.map((capability) => ({
    ...
    // The registrar derives the annotation hint set from the effect class,
    // identically for pracht and standalone callers. (destructive never
    // reaches WebMCP.)
