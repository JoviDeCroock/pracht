...
import { resolveMarkdownAliasPaths } from "@pracht/core/server";
import {
  CLIENT_BROWSER_PATH,
...
function matchesResolvedRoute(
  ...
  if (options.app && options.matchAppRoute) {
    for (const pagePathname of resolveMarkdownAliasPaths(pathname, options.app.markdown)) {
      if (options.matchAppRoute(options.app, pagePathname)) return true;
    }
  }
