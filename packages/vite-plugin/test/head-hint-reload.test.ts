...
import { reachesRouteHintedModule, type HotUpdateModuleLike } from "../src/head-hint-reload.ts";
...
describe("reachesRouteHintedModule", () => {
  it("reports a head-bearing module reached through its importers", () => {
    ...
    expect(reachesRouteHintedModule([fonts], ROOT, HINTS)).toBe(true);
  });

  it("reports the changed module itself by default", () => {
    expect(reachesRouteHintedModule([mod("/app/src/routes/greeting.tsx")], ROOT, HINTS)).toBe(true);
  });
  ...
  it("skips the changed module itself when it is a route source", () => {
    expect(
      reachesRouteHintedModule([mod("/app/src/routes/greeting.tsx")], ROOT, HINTS, {
  ...
  it("still walks importers when skipping the changed module", () => {
    ...
    expect(reachesRouteHintedModule([shared], ROOT, HINTS, { startAtImporters: true })).toBe(true);
  });

  it("returns false when nothing in the graph exports head", () => {
    expect(reachesRouteHintedModule([mod("/app/src/routes/pricing.tsx")], ROOT, HINTS)).toBe(false);
  });
  ...
  it("terminates on an import cycle", () => {
    ...
    expect(reachesRouteHintedModule([a], ROOT, HINTS)).toBe(false);
  });

  it("strips a query from the module id when no file is set", () => {
    expect(
      reachesRouteHintedModule(
