...
export function createStaticServerEntryModule(options: StaticAdapterOptions = {}): string {
  ...
  return [
    ...
    "  // Work the page registers with `waitUntil()` finishes before the build",
    "  // moves on; each task arrives guarded, so none of them reject.",
    "  const backgroundWork = [];",
    "  try {",
    "    return await renderNotFoundDocument((task) => backgroundWork.push(task));",
    "  } finally {",
    "    while (backgroundWork.length > 0) await Promise.allSettled(backgroundWork.splice(0));",
    "  }",
    "}",
    "",
    "async function renderNotFoundDocument(waitUntil) {",
    "  let renderError;",
    ...
    "        console.error(`[pracht] waitUntil error at ${requestPath}:`, error);",
    "        return;",
    "      }",
    "      renderError = error;",
    "    },",
