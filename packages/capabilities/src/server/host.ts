 ...
 * into a Hono route, a Next.js route handler, or a Cloudflare Worker in front
 * of the rest of the application. Node frameworks need their normal
...
export interface CreateCapabilityHostOptions<TContext = Record<string, unknown>> {
   ...
   * owned `fetch()` request when no explicit context is passed. Unrelated URLs
   * fall through before this runs. Return a fresh object per request —
   * contexts must not be reused across requests.
   */
...
export function createCapabilityHost<TContext = Record<string, unknown>>(
  ...
  return {
    ...
    async fetch(request, init = {}) {
      ...
      // Prove ownership before creating application context, verifying agent
      // signatures, or binding an active host. `fetch()` is designed for
      // wildcard middleware mounts, so an unrelated request must be a true
      // no-op before it falls through to the embedding application.
      let match: ResolvedCapability | undefined;
      if (!isMcpRequest) {
        ...
        match = matchCapabilityRoute(capabilities, url.pathname);
        if (!match) {
          // Unmatched requests under the generated capability prefix belong to
          // the host, but need no application context or authentication work.
      ...
      // `isMcpRequest` and `match` exhaust the owned paths above.
      return null;
    },
...
/**
 * Whether a URL could address one of the registered capabilities without
 * resolving the full runtime registry. Generated paths share one prefix;
 * custom HTTP paths are present on the already-created capability objects.
 * This keeps unrelated wildcard traffic from evaluating capability metadata.
 */
