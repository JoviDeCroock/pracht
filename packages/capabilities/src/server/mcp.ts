...
import { hasWebBotAuthIdentitySource } from "./agent-auth.ts";
import {
...
export async function handleMcpMetadataRequest(
  ...
  const { handleMcpResourceMetadataRequest } = await import("./mcp-auth.ts");
...
async function authenticateMcp<TContext>(
  ...
  const { authenticateMcpRequest, bindMcpTokenContext } = await import("./mcp-auth.ts");
