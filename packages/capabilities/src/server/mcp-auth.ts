 ...
 *
 * Loaded lazily from `mcp.ts`: an app that serves MCP without
 * `agents.mcp.auth` never imports this module.
 */

import { mcpResourceMetadataUrl } from "./mcp-config.ts";
import { normalizeModulePath } from "./registry.ts";
import type {
...
export { bindMcpTokenContext } from "./agent-context.ts";
...
function resolveMcpTokenVerifierImporter(
  file: string,
  registry: CapabilityModuleRegistry,
): ModuleImporter {
