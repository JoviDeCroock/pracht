...
/**
 * The annotation set pracht advertises for an effect class, identical across
 * the WebMCP and remote MCP projections so one capability presents one
 * contract on both agent transports. `write` deliberately omits
 * `destructiveHint`: the effect class does not prove the operation is
 * additive, so the host's conservative default applies.
 */
export function webmcpToolAnnotations(
  ...
  return {
    ...
    ...(effect === "read" ? { destructiveHint: false } : {}),
