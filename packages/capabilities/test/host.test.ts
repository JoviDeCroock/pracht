...
describe("createCapabilityHost HTTP dispatch", () => {
  ...
  it("does no context or trust work for URLs outside the capability surface", async () => {
    const createContext = vi.fn(() => {
      throw new Error("unrelated requests must fall through before context creation");
    });
    const host = createCapabilityHost({
      capabilities: { "notes.search": searchCapability() },
      agents: { webBotAuth: { directories: ["https://agent.example"] } },
      createContext,
    });
    ...
    await expect(
      host.fetch(
        new Request(`${ORIGIN}/about`, {
          headers: { "signature-input": 'sig=("@method");keyid="unknown"' },
        }),
      ),
    ).resolves.toBeNull();
  ...
  it("answers an unknown generated capability path without creating context", async () => {
    const createContext = vi.fn(() => {
      throw new Error("404s do not need application context");
    });
    const host = createCapabilityHost({
      capabilities: { "notes.search": searchCapability() },
      createContext,
    ...
    const response = await host.fetch(post("/api/capabilities/notes/missing", {}));
...
describe("createCapabilityHost remote MCP", () => {
  ...
  it("answers initialize and tools/list", async () => {
    ...
    expect(listBody.result.tools[0].annotations).toEqual({
      readOnlyHint: true,
...
describe("createCapabilityHost registration errors", () => {
  ...
  it("does not resolve a broken capability for an unrelated URL", async () => {
    const host = createCapabilityHost({
      capabilities: {
        "notes.search": { kind: "not-a-capability" } as never,
      },
