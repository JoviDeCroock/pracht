...
- Keeps `args.waitUntil()` work alive after the response and drains it on graceful shutdown: the generated server waits for it on `SIGTERM`/`SIGINT` up to `nodeAdapter({ shutdownTimeoutMs })` (default 10s); custom servers call `await handler.drain(ms)` after `server.close()`
