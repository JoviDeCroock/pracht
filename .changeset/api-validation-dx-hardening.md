...
- `apiFetch()` query typing now rejects, at compile time, query keys whose schema input has no string representation (e.g. `z.number()`): query values cross the wire as strings, so those schemas could never validate a real request. String-accepting inputs (`z.coerce.number()`, enums, unions with a string arm) pass through unchanged.
