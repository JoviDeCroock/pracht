...
- `apiFetch()` is a typed fetch client for API routes. With `pracht typegen`, it checks paths, methods, params, bodies, and queries at compile time and returns the handler's response type (`undefined` for bodyless `HEAD` responses); without generated types it stays usable with `unknown` payloads. `GET` and `HEAD` request bodies are rejected. Non-2xx responses throw `ApiFetchError`, exposing normalized validation `issues` when present.
