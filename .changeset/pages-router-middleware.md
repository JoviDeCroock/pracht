...
`_middleware` file, multiple root-level files, and a module without a
`middleware` export all fail the build, `pracht doctor`, and `pracht verify`
(the runtime already refused to serve routes whose middleware lacks the
export). The client bundle excludes `_middleware` files from the pages route
glob so server-only middleware code is never emitted as a browser-loadable
chunk.
