...
- **SPA routes** — static `render: "spa"` routes must be loaderless. Their shell HTML is prerendered, they boot without pending route state, and in-app navigation renders them entirely client-side. Use browser-side fetches to external APIs for live data. `staticAdapter({ fallback: "200.html" })` emits an SPA fallback document for hosts that can rewrite unmatched URLs, enabling deep links into dynamic SPA routes.
