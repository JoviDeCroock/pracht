...
| `hydration: "full"` | **17.5 KB** | 42.9 KB | The above plus the client router: navigation, prefetching, loader fetches. |
| `hydration: "full"`, prefetching off | **16.0 KB** | 41.9 KB | Full hydration with `client: { prefetch: false }`. |
| `hydration: "full"`, navigation guards off | **17.2 KB** | 42.0 KB | Full hydration with `client: { navigationGuards: false }`. |
| `hydration: "full"` + `preact/compat` | **18.3 KB** | 45.2 KB | Full hydration with the React compatibility layer in the graph. |
