...
| `hydration: "full"` | **16.8 KB** | 41.4 KB | The above plus the client router: navigation, prefetching, loader fetches. |
| `hydration: "full"`, prefetching off | **15.4 KB** | 40.5 KB | Full hydration with `client: { prefetch: false }`. |
| `hydration: "full"`, navigation guards off | **16.6 KB** | 40.5 KB | Full hydration with `client: { navigationGuards: false }`. |
| `hydration: "full"` + `preact/compat` | **18.2 KB** | 45.9 KB | Full hydration with the React compatibility layer in the graph. |
