...
`navigate()`, and its output for `useSearch(routeId)`. Navigation requires
`search` when the schema input does not accept an empty record, and rejects
input fields that cannot accept the string values sent over the URL. Routes
