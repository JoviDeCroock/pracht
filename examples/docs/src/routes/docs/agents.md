...
- **May they do this?** Effect classes are load-bearing, not documentation. An incoming `destructive` HTTP call cannot run on first contact — the server answers `409 confirmation_required` with a token bound to this caller, operation, and exact input. Remote MCP carries destructive tools only when the app opts in with `agents.mcp.destructive` and registers a durable approval store — the same exchange, with the token in `_meta` — and nested MCP calls re-apply the callee's `agentPolicy` while refusing destructive effects nobody confirmed.
