...
- **May they do this?** Effect classes are load-bearing, not documentation. An incoming `destructive` capability call cannot run on first contact — the server answers `409 confirmation_required` with a token bound to this caller, operation, and exact input. Trusted server composition is not re-gated, so an exposed capability that composes destructive work must own its authorization or approval gate.
