...
- Pages-router apps have a single middleware seam: `pracht generate middleware --name _middleware` scaffolds the root `src/pages/_middleware.ts`, which runs on every page route (API routes are not wrapped). Other names error in pages mode; `generate shell` stays manifest-only (`_app.tsx` is the pages shell).
