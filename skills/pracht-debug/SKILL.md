...
- HMR: changes to `src/routes.ts` restart the dev server (`server.restart()`, not a browser-side full reload). Route, shell, and island components use Preact Fast Refresh; route/shell edits and client-reachable dependencies of loader-bearing routes also re-fetch active route data, with rapid saves coalesced so the newest result settles last and a failed latest refresh falls back to a reload. Adding or removing a route `loader` or route/shell `head` export reloads so the generated client hints stay current, and editing a route/shell that exports document `headers()` reloads because CSP and other response headers cannot be patched. Markdown, MDX, `.tsrx`, and `additionalExtensions` formats join the same refresh path after their companion Vite plugin compiles them; compiled formats stay conservatively header-bearing because a transform may synthesize `headers()` from metadata. Other route/shell/middleware/API/server/islands changes invalidate the server module.
