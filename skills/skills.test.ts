...
function commandTree(commandFile: string): CliCommand {
  ...
  for (const statement of sourceFile.statements) {
    if (
      ...
      importedCommands.set(
        statement.importClause.name.text,
        resolve(dirname(commandFile), statement.moduleSpecifier.text.replace(/\.js$/, ".ts")),
      );
      continue;
    }
  ...
  function parseCommand(expression: ts.Expression): CliCommand {
    if (ts.isIdentifier(expression)) {
      const importedFile = importedCommands.get(expression.text);
      if (importedFile) return commandTree(importedFile);
    }
...
function validatePrachtInvocation(invocation: PrachtInvocation): string[] {
  ...
  while (cursor < invocation.argv.length) {
    ...
    for (const flag of command.flags) allowedFlags.add(flag);
  ...
  for (const token of invocation.argv.slice(cursor)) {
    ...
    if (!allowedFlags.has(flag) && !allowedFlags.has(normalized)) {
      problems.push(
        `flag "--${flag}" (from \`${invocation.source}\`) is not defined by "pracht ${commandPath.join(" ")}"; known: ${[...allowedFlags].map((known) => `--${known}`).join(", ")}`,
      );
    }
  }
  return problems;
...
describe("drift-guard source extraction", () => {
  ...
  it("extracts citty flags in their nested command context", () => {
    ...
    const verify = CLI_SUBCOMMANDS.get("verify");
    expect(verify?.subcommands.get("webmcp")?.flags).toContain("start");
  });
